        ...
        private void AutoScaleControl(Control ctl, float wScale, float hScale)
        {
            ...
            { //**放在这里，是先缩放控件的子控件，后缩放控件本身
              ...
              //   AutoScaleControl(c, wScale, hScale);//窗体内其余控件还可能嵌套控件(比如panel),要单独抽出,因为要递归调用
                if (c.Name == "pictureBox1" || c.Name== "pictureBox2")
                {                   
                    c.Left = oldCtrl[ctrlNo].Left; ;//新旧控件之间的线性比例。控件位置只相对于窗体，所以不能加 + wLeft1
                    c.Top = oldCtrl[ctrlNo].Top;//
                    c.Width = oldCtrl[ctrlNo].Width;//只与最初的大小相关，所以不能与现在的宽度相乘 (int)(c.Width * w);
